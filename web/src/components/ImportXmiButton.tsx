...
export function ImportXmiButton({ projectId }: ImportXmiButtonProps) {
  ...
  async function handleFileChange(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      // que pisaría el multipart/form-data con boundary que el browser arma solo
      // para un FormData -- acá solo hace falta el header de autenticación.
      const res = await fetch(`${API_BASE}/projects/${projectId}/import-xmi`, {
        method: 'POST',
      ...
      if (!res.ok) {
        setErrorMessage(
          res.status === 403
            ? 'Necesitás ser OWNER o EDITOR del proyecto para importar un XMI'
            : `No se pudo importar el XMI (${res.status})`,
        )
        setStatus('error')
        return
      }
